  ...
  cliParam(0)
{
  ...
  portAudioIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portAudioIn.ready = true;
  zmq_sockets.push_back(&portAudioIn);
  portInfraredIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portInfraredIn.ready = true;
  zmq_sockets.push_back(&portInfraredIn);
  portLeftWheelOut.sock = new zmq::socket_t(*context, ZMQ_REQ);
  portLeftWheelOut.ready = true;
  zmq_sockets.push_back(&portLeftWheelOut);
}

ClusterModule::~ClusterModule() {
  delete cliParam;
  delete portAudioIn.sock;
  delete portInfraredIn.sock;
  delete portLeftWheelOut.sock;
}
...
void ClusterModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/clustermodule" + cliParam->module_id + "/audio";
    portAudioIn.name = zmqPortName;
    zmqPortName = "/resolve" + portAudioIn.name;
    pns_record record;
    ...
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portAudioIn.sock->bind(zmqPortName.c_str());
  }
  {
    // incoming port, function as client
    zmqPortName = "/clustermodule" + cliParam->module_id + "/infrared";
    portInfraredIn.name = zmqPortName;
    zmqPortName = "/resolve" + portInfraredIn.name;
    pns_record record;
    record.name = zmqPortName;
    zmqSs.clear(); zmqSs.str("");
    zmqSs << getpid();
    ...
    zmqPortName = zmqSs.str();
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portInfraredIn.sock->bind(zmqPortName.c_str());
  }
  {
    // outgoing port, function as server
    zmqPortName = "/clustermodule" + cliParam->module_id + "/leftwheel";
    portLeftWheelOut.name = zmqPortName;
    zmqPortName = "/resolve" + portLeftWheelOut.name;
    pns_record record;
    record.name = zmqPortName;
    zmqSs.clear(); zmqSs.str("");
    zmqSs << getpid();
...
void ClusterModule::readCommands() {
  while (true) {
    HandleCommand();
  }
}
...
void ClusterModule::Resolve(pns_record & record) {
...
void ClusterModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool ClusterModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* ClusterModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void ClusterModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void ClusterModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* ClusterModule::GetSocket(std::string name) {
...
long_seq* ClusterModule::readAudio(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
  char *reply = GetReply(portAudioIn.sock, portAudioIn.ready, blocking, reply_size);
  if (!portAudioIn.ready) {
    delete [] reply;
    pthread_mutex_unlock(&cmdMutex);
    return NULL;
  }
  ...
  while (std::getline(ss, item, delim)) {
    ...
    ssItem << item;
    ssItem >> itemVal;
    portAudioValue.push_back(itemVal);
  }
  delete [] reply;
  return &portAudioValue;
}

int* ClusterModule::readInfrared(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
  char *reply = GetReply(portInfraredIn.sock, portInfraredIn.ready, blocking, reply_size);
  if (!portInfraredIn.ready) {
    delete [] reply;
    pthread_mutex_unlock(&cmdMutex);
    return NULL;
  }
...
bool ClusterModule::writeLeftWheel(const int output) {
  ...
  pthread_mutex_lock(&cmdMutex);
  bool state = portLeftWheelOut.ready;
  SendRequest(portLeftWheelOut.sock, state, false, ss.str());
  if (state) portLeftWheelOut.ready = false;
  if (!portLeftWheelOut.ready) {
    bool ack_state = true;
    ReceiveAck(portLeftWheelOut.sock, ack_state, true);
    if (ack_state) {
      portLeftWheelOut.ready = true;
      pthread_mutex_unlock(&cmdMutex);
      return true;
