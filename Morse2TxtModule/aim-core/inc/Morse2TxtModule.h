...
#ifndef MORSE2TXTMODULE_H_
#define MORSE2TXTMODULE_H_
...
private:
  ...
  zmq_socket_ext portTxtOut;
  
  std::string portMorseValue;
  zmq_socket_ext portMorseIn;
  
  static void* readCommandsHelper(void* object) {
    ((Morse2TxtModule*)object)->readCommands();
    return NULL;
  }
...
public:
  ...
  // Default destructor
  virtual ~Morse2TxtModule();
  
  // Extend this with your own code, first call Morse2TxtModule::Init(name);
  ...
  /**
   * The "writeTxt" function sends stuff over a zeromq REQ socket. It works as a server. It cannot be blocking because this
  ...
  bool writeTxt(const std::string output);
  
  /**
   * The "readMorse" function receives stuff over a zeromq REP socket. It works as a client. It is better not
