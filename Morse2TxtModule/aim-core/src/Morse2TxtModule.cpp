...
Morse2TxtModule::Morse2TxtModule():
  ns_socket(NULL),
  cmd_socket(NULL),
  debug(0),
  cliParam(0)
{
  const char* const channel[2] = {"writeTxt", "readMorse"};
  cliParam = new Param();
  context = new zmq::context_t(1);
  pthread_mutex_init(&cmdMutex, NULL);
  pthread_create(&cmdThread, 0, readCommandsHelper, this);
  portTxtOut.sock = new zmq::socket_t(*context, ZMQ_REQ);
  portTxtOut.ready = true;
  zmq_sockets.push_back(&portTxtOut);
  portMorseIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portMorseIn.ready = true;
  zmq_sockets.push_back(&portMorseIn);
}

Morse2TxtModule::~Morse2TxtModule() {
  delete cliParam;
  delete portTxtOut.sock;
  delete portMorseIn.sock;
}

void Morse2TxtModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/morse2txtmodule" + cliParam->module_id + "/txt";
    portTxtOut.name = zmqPortName;
    zmqPortName = "/resolve" + portTxtOut.name;
    pns_record record;
  ...
  }
  {
    ...
    zmqPortName = "/morse2txtmodule" + cliParam->module_id + "/morse";
    portMorseIn.name = zmqPortName;
    zmqPortName = "/resolve" + portMorseIn.name;
    pns_record record;
    record.name = zmqPortName;
    zmqSs.clear(); zmqSs.str("");
    zmqSs << getpid();
    ...
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portMorseIn.sock->bind(zmqPortName.c_str());
  }
}

void Morse2TxtModule::readCommands() {
  while (true) {
    HandleCommand();
  }
}
...
void Morse2TxtModule::Resolve(pns_record & record) {
...
void Morse2TxtModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool Morse2TxtModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* Morse2TxtModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void Morse2TxtModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void Morse2TxtModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void Morse2TxtModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* Morse2TxtModule::GetSocket(std::string name) {
...
bool Morse2TxtModule::writeTxt(const std::string output) {
  std::stringstream ss; ss.clear(); ss.str("");
  ss << output; // very dirty, no endianness, etc, just use the stream operator itself
  pthread_mutex_lock(&cmdMutex);
  bool state = portTxtOut.ready;
  SendRequest(portTxtOut.sock, state, false, ss.str());
  if (state) portTxtOut.ready = false;
  if (!portTxtOut.ready) {
    bool ack_state = true;
    ReceiveAck(portTxtOut.sock, ack_state, true);
    if (ack_state) {
      portTxtOut.ready = true;
      pthread_mutex_unlock(&cmdMutex);
      return true;
...
std::string* Morse2TxtModule::readMorse(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
  char *reply = GetReply(portMorseIn.sock, portMorseIn.ready, blocking, reply_size);
  if (!portMorseIn.ready) {
    delete [] reply;
    pthread_mutex_unlock(&cmdMutex);
    return NULL;
  }
  ...
  SendAck(portMorseIn.sock, portMorseIn.ready);
  pthread_mutex_unlock(&cmdMutex);
  if (reply_size < 1) std::cerr << "Error: Reply is not large enough to store a value!" << std::endl;
  portMorseValue = std::string(reply);
  delete [] reply;
  return &portMorseValue;
}
